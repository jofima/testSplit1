...
public class FindPattern {
	...
	public FindPattern(String findPat) {
		...
		String pattern1 = "(Nº\\s\\d+)|(,N\\d+)|(,N\\s\\d+)|(\\sN\\s\\d+)|(nº\\d+)|(\\sNº.\\s\\d+)|(Nº.\\d+)";
		String pattern2 = "(LOTE\\s\\d+)|(lOTE\\s\\d+)|(LOTE.\\d+)|(LOTE\\d+)|(Lt\\s\\d+)|(LT\\d+)|(LT.\\d+)|(LT\\s[A-Z])|(LOTE\\s[A-Z])";
		String pattern3 = "(,\\s\\d+$)|(\\s\\d+\\w$)|(\\s\\d+,)|(\\sN\\d+$)|(\\d+$)|(\\s\\d+\\s)";
		String pattern4 = "(RUA\\s\\d+)|(AVENIDA\\s\\d+)|()";
//				+ ")|(AV.\\s\\d*)|(\\srua\\s\\d*)"
